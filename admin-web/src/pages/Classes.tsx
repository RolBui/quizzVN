...
function TeacherAvatar({ classroom }: { classroom: AdminClass }) {
  if (classroom.teacher_avatar_url) {
    return (
      <img
        src={classroom.teacher_avatar_url}
        alt={classroom.teacher_name || "Giáo viên"}
        className="w-10 h-10 rounded-full object-cover"
      />
    );
  }
  return (
    <div className="w-10 h-10 rounded-full bg-surface-container flex items-center justify-center text-on-surface font-semibold text-sm">
    ...
    <div className="p-4 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {overview.metrics.map((metric) => (
          <div
            key={metric.key}
            className="bg-surface-container-lowest rounded-xl p-5 border border-surface-variant shadow-(--shadow-level-1)"
          >
            ...
            <p className="text-xs text-on-surface mt-1">{metric.subtext}</p>
          </div>
        ))}
      </div>

      <div className="border-b border-surface-variant flex gap-4 mt-2">
        <button className="pb-3 text-sm font-semibold text-on-surface border-b-2 border-primary">
          Tất cả Lớp học
        </button>
      </div>
        ...
        {filteredClasses.map((classroom) => (
          ...
          >
            <div className="h-24 bg-surface-container-low relative flex items-center justify-between px-5">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-on-surface font-bold">
                  Mã lớp
                </p>
            ...
            <div className="p-5 flex flex-col flex-1">
              ...
              <p className="text-sm text-on-surface line-clamp-2 min-h-10">
              ...
              <div className="flex items-center gap-3 my-5">
                ...
                <div>
                  <p className="text-sm font-semibold text-on-surface">
                    {classroom.teacher_name || "Chưa gán giáo viên"}
                  </p>
                  <p className="text-xs text-on-surface">Giáo viên phụ trách</p>
                </div>
              </div>
              ...
              <div className="mt-auto grid grid-cols-3 pt-4 border-t border-surface-variant gap-4">
                ...
                <div>
                  <p className="text-xs font-semibold text-on-surface flex items-center gap-1.5 mb-1">
                    <FileText className="w-3.5 h-3.5" /> Bài thi
                  </p>
                  <p className="text-xl font-bold text-on-surface leading-none">
